...
export default function Hero() {
  ...
  useEffect(() => {
    // Sekojam valodas maiņai
    const updateLang = () => setLang(getCurrentLanguage().toUpperCase());
    updateLang();
    
    // Klausāmies uz iespējamām valodas maiņas darbībām (ja utils to atbalsta)
  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden bg-gradient-to-br from-[#101e33] via-[#1a2d4a] to-[#101e33]">
      ...
      <div className="absolute inset-0 opacity-5 pointer-events-none">
        <svg className="absolute top-20 left-10 w-32 h-48" viewBox="0 0 100 150" fill="none" aria-hidden="true">
        ...
        <div className="absolute bottom-20 right-20 w-48 h-64">
          <svg viewBox="0 0 100 150" className="w-full h-full" fill="none" aria-hidden="true">
      ...
      <div className="container mx-auto px-4 py-32 relative z-10">
        <div className="max-w-5xl mx-auto text-center">
          ...
          {/* Main Title */}
          <h1 className="text-4xl md:text-7xl font-bold text-white mb-6 leading-tight break-words px-2">
            {current.title}{' '}
            <span className="text-[#b22234] inline-flex items-center gap-2">
              {current.span}
              <svg className="w-10 h-6 md:w-12 md:h-8 inline-block shadow-lg" viewBox="0 0 60 30" fill="none" role="img">
                <title>UK Flag</title>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-4xl mx-auto px-4">
            {[
              ...
              <div key={idx} className="bg-white/5 backdrop-blur-md p-8 rounded-2xl border border-white/10 hover:border-[#b22234]/50 transition-colors shadow-xl">
                <p className="text-5xl font-bold text-[#b22234] mb-3">{stat.val}</p>
